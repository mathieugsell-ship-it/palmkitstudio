...
const SEG_H = 0.7;
const W_BOTTOM = 0.44; // slimmer trunk (was 0.6) — frees up the centre of the island
const W_TOP = 0.22; // slimmer top (was 0.3); the crown still overhangs it cleanly
...
// Shared crown attachment point of all fronds (scene-local = world here, since
// the diorama's idle rotation is paused). The gentle wind sway (v37) rotates
// each frond about this origin so the tip moves most and the trunk stays still.
// Includes the v41 island/palm pushback offset so the sway pivot tracks the palm.
export const FROND_ORIGIN: Vec3 = [ARC + ISLAND_OFFSET[0], CROWN_CENTER_Y + 0.02, ISLAND_OFFSET[1]];
...
export function buildPalm(config: PalmConfig): PalmModel {
  ...
  // ---- vertex field — TOLERANCE-MERGED so there is exactly ONE dot per real
  // corner. Stacked tapered trunk blocks (and chained frond leaflets) leave two
  // near-coincident corners at every seam; a fixed-decimal key never merged
  // them, so the seams showed *doubled* dots that visually merged/split as the
  // palm rotated (reading as points "appearing/disappearing"). We keep the
  // FIRST real corner in each cluster, so every dot sits exactly on a true
  // geometry vertex. Computed once here (memoized upstream).
  const MERGE_TOL = 0.07;
  const tol2 = MERGE_TOL * MERGE_TOL;
  const vertices: Vec3[] = [];
  for (const b of blocks) {
    ...
    for (const c of boxCorners(b.size, pos, quat)) {
      ...
      if (!dup) vertices.push(c);
    }
  }

  // v41: push the whole palm back with the island (shared ISLAND_OFFSET). Applied
  // last so the local geometry/vertex-merge above is unaffected — only the final
  // world (x,z) of every block, vertex and anchor is translated.
  const [OX, OZ] = ISLAND_OFFSET;
  if (OX !== 0 || OZ !== 0) {
    for (const b of blocks) b.position = [b.position[0] + OX, b.position[1], b.position[2] + OZ];
    for (let i = 0; i < vertices.length; i++) vertices[i] = [vertices[i][0] + OX, vertices[i][1], vertices[i][2] + OZ];
    for (const k of Object.keys(anchorsById)) {
      const a = anchorsById[k];
      anchorsById[k] = [a[0] + OX, a[1], a[2] + OZ];
    }
  }
