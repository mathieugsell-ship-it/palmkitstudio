...
export interface PalmConfig {
  ...
  rippleSpeed: number;
  rippleK: number;
  /** Palm "construction" build-in animation (one normalized 0→1 timeline). */
  build: {
    duration: number; // total seconds of the assembly
    edgesAt: number; // phase-start offsets on the 0→1 timeline (points start 0)
    facesAt: number;
    heightStagger: number; // span of the bottom-to-top sweep within a phase
    revealWin: number; // per-element fade window
    rotateEaseIn: number; // seconds to glide idle rotation in after the build
  };
}
...
export const DEFAULT_CONFIG: PalmConfig = {
  ...
  build: {
    // Mostly-sequential phases with a slight overlap: points 0→0.43,
    // edges 0.28→0.71, faces 0.55→0.98 — each still climbing bottom-to-top.
    duration: 2.2,
    edgesAt: 0.28,
    facesAt: 0.55,
    heightStagger: 0.3,
