...
interface Props {
  ...
  // STEP 9 blueprint UI: screen-projection drives a DOM connector line + hover
  // label chip (desktop enhancement; degrades to nothing if refs are absent).
  desktop?: boolean;
...
export function SceneBuild({
  ...
  const anchorGeo = useMemo(() => new THREE.SphereGeometry(0.1, 16, 16), []);
  const hitGeo = useMemo(() => new THREE.SphereGeometry(0.26, 10, 10), []);
  ...
  // Expanding "ping/radar" ring per node — the universal interactive signal.
  // Billboarded to the camera each frame (countering the palm's rotation).
  const ringGeo = useMemo(() => new THREE.RingGeometry(0.82, 1.0, 40), []);
  const ringMats = useMemo(
    () =>
      scene.anchors.map(
        () =>
          new THREE.MeshBasicMaterial({
            color: new THREE.Color(config.colors.vertex).multiplyScalar(1.35), // >1 → blooms
            toneMapped: false,
            transparent: true,
            opacity: 0,
            depthWrite: false,
  ...
  const billboard = useMemo(() => ({ qCam: new THREE.Quaternion(), qRoot: new THREE.Quaternion() }), []);
  ...
  const nodeAnim = useRef({
    ...
    conn: 0, // 0→1 connector-line draw progress
    connIdx: -1, // anchor index the connector points at (kept during retract)
  ...
  useEffect(
    ...
    [faceGeoms, edgeGeoms, faceMats, edgeMats, pointGeo, pointMat, anchorGeo, hitGeo, hitMat, anchorMats, ringGeo, ringMats],
  );
  ...
  useFrame((state, delta) => {
    ...
    // ---- ping/radar rings (overview only) ---------------------------------
    let billboardReady = false;
    if (rootRef.current) {
      rootRef.current.getWorldQuaternion(billboard.qRoot);
      state.camera.getWorldQuaternion(billboard.qCam);
      billboard.qRoot.invert().multiply(billboard.qCam); // local quat that faces the camera
      billboardReady = true;
    }
    const pingVis = pulse && !reducedMotion ? A.appear * (1 - A.anySel) : 0;
    for (let i = 0; i < scene.anchors.length; i++) {
      const ring = ringRefs.current[i];
      if (!ring) continue;
      if (billboardReady) ring.quaternion.copy(billboard.qRoot);
      const phase = (t * 0.5 + i * 0.16) % 1; // looping, staggered per node
      ring.scale.setScalar(0.13 + 0.42 * phase); // expands outward (world units)
      (ring.material as THREE.MeshBasicMaterial).opacity = Math.sin(phase * Math.PI) * 0.5 * pingVis;
    }

    // ---- screen projection → DOM connector line + hover label (desktop) ----
    if (labelRef?.current || connectorRef?.current) {
      const rect = state.gl.domElement.getBoundingClientRect();
      const project = (mesh: THREE.Mesh) => {
        mesh.getWorldPosition(scratch.p).project(state.camera);
        return {
          x: rect.left + (scratch.p.x * 0.5 + 0.5) * rect.width,
          y: rect.top + (-scratch.p.y * 0.5 + 0.5) * rect.height,
        };
      };

      if (labelRef?.current && desktop) {
        const idx = hovered != null ? scene.anchors.findIndex((a) => a.id === hovered) : -1;
        const mesh = idx >= 0 ? anchorRefs.current[idx] : null;
        if (mesh) {
          const sp = project(mesh);
          labelRef.current.style.transform = `translate(${sp.x}px, ${sp.y}px) translate(-50%, -150%)`;
        }
      }

      if (connectorRef?.current) {
        const line = connectorRef.current;
        const open = desktop && selected != null;
        if (selected != null) A.connIdx = scene.anchors.findIndex((a) => a.id === selected);
        A.conn += ((open ? 1 : 0) - A.conn) * ease;
        const mesh = A.connIdx >= 0 ? anchorRefs.current[A.connIdx] : null;
        const anchor = panelAnchorRef?.current;
        if (mesh && anchor && A.conn > 0.01) {
          const sp = project(mesh);
          line.setAttribute('x1', String(sp.x));
          line.setAttribute('y1', String(sp.y));
          line.setAttribute('x2', String(lerp(sp.x, anchor.x, A.conn)));
          line.setAttribute('y2', String(lerp(sp.y, anchor.y, A.conn)));
          line.style.opacity = String(Math.min(1, A.conn * 4));
        } else {
