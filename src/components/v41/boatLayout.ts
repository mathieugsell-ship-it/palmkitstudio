...
// ---- placement (tunable) — v40 composition: brought to the foreground
// CENTRE-RIGHT (moved right from the old front-left) to balance the pitons+sun
// on the left, kept as far right as the front water ring allows with the whole
// hull + shaft still afloat (verified). Prow angled toward the viewer.
const CX = 0.6;
const CY = -0.42; // waterline reference
const CZ = 2.2; // foreground open water, centre-right
const PHI = 5.88; // yaw about Y (radians) — prow angled toward the viewer

// Rigid-body anchor for the gentle idle motion (v36): the boat pitches/rolls
// about its centre at the WATERLINE, and BOAT_YAW gives its longitudinal/lateral
// axes so the rock is along the hull, not the world axes. (Geometry unchanged.)
export const BOAT_PIVOT: Vec3 = [CX, CY, CZ];
export const BOAT_YAW = PHI;
