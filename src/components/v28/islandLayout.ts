...
export const outlineR = (theta: number) => {
  ...
  // One-sided smooth bulge (tight cos⁶ falloff so it concentrates straight back
  // into the depth and never reaches the left or the boat), with a little extra
  // wobble so the extended side stays organic rather than a clean circular arc.
  const dth = Math.atan2(Math.sin(theta - BULGE_DIR), Math.cos(theta - BULGE_DIR));
  const bulge = BULGE * Math.pow(Math.max(0, Math.cos(dth)), 6) * (1 + 0.08 * Math.sin(4 * theta + 0.9));
