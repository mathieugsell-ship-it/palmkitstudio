...
export interface PalmConfig {
  ...
  build: {
    ...
    finalEdgeOpacity: number; // resting edge opacity for SOLID objects (softened, still visible)
    waterEdgeOpacity: number; // resting edge opacity for WATER (much lower — recedes into the turquoise)
...
export const COLORS = {
  ...
  edgeBuild: '#5FE3D6',
  edgeFinal: '#D7E2E6', // resting edge tint for SOLID objects (cool grey-white)
  edgeWater: '#5FB6BC', // resting edge tint for WATER — a soft teal close to the water so the grid recedes (v45)
  ...
  waterBody: '#07A7B0', // main water — richer turquoise (Option C, more vivid)
  waterDeep: '#086E7B', // deeper / far blocks — deep jewel teal (keeps depth read)
  waterShallow: '#2FD4C0', // bright jewel-aqua shallows meeting the shore
...
export const DEFAULT_CONFIG: PalmConfig = {
  ...
  waterOpacity: 0.82, // water body — less see-through (v43) so the turquoise reads vivid, still slightly translucent
  shipOpacity: 1.0, // cruise ship — fully opaque (v44) so it stands out from the vivid water behind it
  ...
  build: {
    ...
    rotateEaseIn: 0.6,
    edgeBuildBoost: 2.0,
    finalEdgeOpacity: 0.55, // softened from 0.85 — solids keep faint voxel edges, less white grid
    waterEdgeOpacity: 0.15, // water edges strongly reduced so the turquoise reads, not a grid
