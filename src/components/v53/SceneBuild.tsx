...
// 3D anchor nudge per object (added to the AABB top-centre) so the pin sits nicely.
const ANCHOR_NUDGE: Record<string, [number, number, number]> = {
  palm: [0.4, 0.25, 0],
  boat: [0, 0.35, 0],
  piton: [0, 0.35, 0],
  ship: [0, 0.4, 0],
...
export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', noHover = false, labels, onHit, onBuildComplete }: Props) {
  ...
  const clickables = useMemo(() => {
    ...
    return Object.entries(acc).map(([group, a]) => {
      ...
      const n = ANCHOR_NUDGE[group] ?? [0, 0.3, 0];
      const anchor: [number, number, number] = [center[0] + n[0], a.max[1] + n[1], center[2] + n[2]];
  ...
  return (
    <group>
      ...
      {/* v53: always-visible service label pins (drei Html, screen-space). Each
          tracks its object's 3D anchor; discreet at rest, intensifies on
          hover/tap. The pill is also clickable (same as tapping the object). */}
      {clickables.map((c) => {
        ...
        return (
          <Html key={`lbl-${c.group}`} position={c.anchor} occlude={false} zIndexRange={[20, 0]} style={{ pointerEvents: 'none' }}>
            <div className={`svc-pin${active ? ' is-active' : ''}`}>
              <button
                type="button"
                className="svc-pill"
