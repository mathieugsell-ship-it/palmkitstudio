...
export function buildWater(config: PalmConfig): WaterModel {
  ...
  // ---- circular water DISC --------------------------------------------------
  // Round the sea into ONE clean disc of radius R_DISC, centred on the islet and
  // big enough to contain the distant pitons toward its back edge. The fine
  // foreground ring above is untouched; here we fill the rest of the disc beyond
  // it. Generated as z-rows clipped to the circle (clean round rim), with cells
  // finer toward the back (perspective recede) and coarser in the near front
  // (mostly below frame). Colour fades paler toward the hazy rim.
  const R_DISC = 9.3;
  const clamp = (v: number, a: number, b: number) => (v < a ? a : v > b ? b : v);
  let z = R_DISC;
  let nf = 0;
  while (z > -R_DISC) {
    const cell = clamp(1.0 + z * 0.05, 0.78, 1.4); // finer toward the back
    const zc = z - cell / 2;
    const halfX = Math.sqrt(Math.max(0, R_DISC * R_DISC - zc * zc)); // clip to circle
    const thk = Math.max(0.18, cell * 0.5);
    for (let x = -halfX; x <= halfX + 1e-6; x += cell) {
      const d = Math.hypot(x, zc);
      const theta = Math.atan2(zc, x);
      if (d <= outlineR(theta)) continue; // never over the island
      if (d <= waterOuter(theta)) continue; // already covered by the fine ring
      const tHaze = clamp((d - waterOuter(theta)) / (R_DISC - 3), 0, 1);
      const seed = Math.round(x * 50 + zc * 30) + 7000 + nf;
      let col = jitter(c.waterDeep, seed);
      col = `#${new THREE.Color(col).lerp(new THREE.Color(c.skyTop), tHaze * 0.5).getHexString()}`;
      blocks.push({
        id: `disc-${nf}`,
