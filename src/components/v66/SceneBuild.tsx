...
export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', noHover = false, labels, onHit, onBuildComplete, focusActive = false, compact = false }: Props) {
  ...
  useFrame((state, delta) => {
    ...
    // ---- v66: palm focus (glide + gentle upscale + frond sway) + recede ----
    // The rest of the world fades out; the palm glides to its focus slot and a
    // gentle upscale, holding with the existing subtle frond sway. Back reverses
    // it (fa → 0). No growth/spin — the décor is the 2D foliage backdrop now.
    {
      ...
      if (builtRef.current && fa > 0.0005 && palmRig.blocks.length) {
        ...
        const t = state.clock.elapsedTime;
        const F = config.frondMotion;
        // 1) all palm blocks → static focus pose (glide to slot + gentle upscale)
        for (const pb of palmRig.blocks) {
          const mesh = meshRefs.current[pb.i];
          if (!mesh) continue;
          focusScratch.to.copy(pb.basePos).sub(cen).multiplyScalar(FOCUS_SCALE).add(cen).add(gl);
          mesh.position.copy(pb.basePos).lerp(focusScratch.to, fa);
          mesh.quaternion.copy(pb.baseQuat);
          mesh.scale.setScalar(1);
        }
        // 2) overlay the gentle frond sway in the focus slot (subtle life)
        if (swayOn && frondData.length) {
          for (const fd of frondData) {
            ...
            for (const bl of fd.blocks) {
              ...
              if (!mesh) continue;
              frondScratch.v.copy(bl.basePos).sub(frondOrigin).applyQuaternion(frondScratch.r).add(frondOrigin);
              focusScratch.to.copy(frondScratch.v).sub(cen).multiplyScalar(FOCUS_SCALE).add(cen).add(gl);
