...
import { buildShip } from './shipLayout';
import type { PalmConfig, Vec3 } from './config';

export type SceneGroup = 'island' | 'palm' | 'water' | 'boat' | 'piton' | 'ship';
...
export function buildScene(config: PalmConfig): SceneModel {
  ...
  for (const set of [palm.blocks, island.blocks, water.blocks, boat.blocks, pitons.blocks, ship.blocks])
    for (const b of set) {
  ...
  const shipY = yRange(ship.blocks.map((b) => b.position[1]));
  const stPit = (y: number) => clamp01((y - pitY[0]) / Math.max(pitY[1] - pitY[0], 1e-3));
  const stShip = (y: number) => clamp01((y - shipY[0]) / Math.max(shipY[1] - shipY[0], 1e-3));
  ...
  // Distant cruise ship — solid voxel, same treatment but a touch hazier
  // (shipOpacity) for atmospheric distance; NO construction points (reserved for
  // the palm). Static; builds bottom-to-top with the pitons.
  for (const b of ship.blocks)
    blocks.push({
      ...
      quaternion: b.quaternion,
      color: b.color,
      finalOpacity: config.shipOpacity,
      o: oOf('ship', stShip(b.position[1])),
      hGlobal: hG(b.position[1]),
      ripple: false,
      ripplePhase: 0,
      baseY: b.position[1],
  ...
  // Palm "construction" nodes removed (v35): the glowing teal vertex dots only
  // ever appeared on the palm (they rested visible while every other element is
  // just faces + thin white edges), so they looked out of place. The palm now
  // builds and rests in the same clean style as the rest — no glowing nodes.
  // (Island/water still use transient scaffold points that dissolve at rest.)
