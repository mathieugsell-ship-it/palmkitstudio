...
import { buildCoconuts } from './coconutLayout';
import type { PalmConfig, Vec3 } from './config';

export type SceneGroup = 'island' | 'palm' | 'water' | 'boat' | 'piton' | 'coconut';
...
export function buildScene(config: PalmConfig): SceneModel {
  ...
  const coconuts = buildCoconuts(config);
  ...
  for (const set of [palm.blocks, island.blocks, water.blocks, boat.blocks, pitons.blocks, coconuts.blocks])
    for (const b of set) {
  ...
  // Coconuts — solid fruit at the crown; same voxel treatment, build WITH the
  // upper palm (palm timing), NO construction points. Rotate with the scene.
  for (const b of coconuts.blocks)
    blocks.push({
      id: `c-${b.id}`,
      group: 'coconut',
      size: b.size,
