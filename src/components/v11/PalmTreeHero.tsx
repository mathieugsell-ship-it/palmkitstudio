...
// Responsive framing for the full-bleed hero. The scene internals (build, dawn,
// settle, ripple, rotation) are untouched — only the CAMERA frames differently:
//   • desktop → the exact validated framing (palm centred, room left/right)
//   • mobile  → pull back + raise the look-at so the palm drops lower & smaller,
//     keeping the upper area clear for the stacked text.
...
export default function PalmTreeHero({ config = DEFAULT_CONFIG }: { config?: PalmConfig }) {
  ...
  const mobile = useMediaQuery('(max-width: 1023px)'); // full-bleed reframing
  ...
  return (
    <div ref={wrapRef} className="h-full w-full" style={{ background: dawnBg }}>
      ...
      >
        ...
        {/* Full-bleed framing: palm centred on desktop, dropped lower on mobile
            so the stacked text stays clear. Scene internals unchanged. */}
