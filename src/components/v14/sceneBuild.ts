...
export type SceneGroup = 'island' | 'palm' | 'water' | 'boat';
...
export function buildScene(config: PalmConfig): SceneModel {
  ...
  for (const set of [palm.blocks, island.blocks, water.blocks, boat.blocks])
    for (const b of set) {
  ...
  const oOf = (group: SceneGroup, st: number) => {
    const base =
      group === 'island' ? B.islandBase : group === 'palm' ? B.palmBase : group === 'water' ? B.waterBase : B.boatBase;
    const span =
      group === 'island' ? B.islandSpan : group === 'palm' ? B.palmSpan : group === 'water' ? B.waterSpan : B.boatSpan;
