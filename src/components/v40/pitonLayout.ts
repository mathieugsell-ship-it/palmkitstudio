...
// ---- placement (tunable) — v40 composition: the two karst stacks move to the
// LEFT and SPREAD APART so the rising sun peeks through the gap between them
// (sunrise between the Phang Nga karsts). Both bases sit on the far back-left
// water (verified); the sun is placed farther behind, in the gap. Only cx/cz
// changed; size/shape/colours/solidity unchanged.
const MAIN: PitonSpec = {
  cx: -6.5, cz: -4.3, levels: 7, segH: 0.42, baseW: 0.5, topW: 1.12,
  rootDepth: 0.32, driftX: -0.2, driftZ: 0.1, seed: 11,
};
const SMALL: PitonSpec = {
  cx: -4.6, cz: -6.2, levels: 4, segH: 0.4, baseW: 0.34, topW: 0.72,
