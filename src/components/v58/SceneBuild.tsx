...
export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', noHover = false, labels, onHit, onBuildComplete }: Props) {
  ...
  // v58: explicit hitbox override (centre + full size) for objects whose padded
  // AABB would otherwise swallow another object on screen. The boat's long
  // propeller shaft inflated its AABB back-and-left (world x to ~-3.85); padded
  // and being the closest object to the camera, that box projected over BOTH the
  // pitons and the cruise ship behind it, so hovering them raycast-hit the boat.
  // This tight box hugs the hull only, so each of the 4 raycasts to itself.
  const HITBOX: Record<string, { center: [number, number, number]; size: [number, number, number] }> = {
    boat: { center: [-0.16, 0.48, 2.34], size: [3.9, 2.3, 2.05] },
  };
  const clickables = useMemo(() => {
    ...
    return Object.entries(acc).map(([group, a]) => {
      ...
      const hb = HITBOX[group];
      const center: [number, number, number] = hb
        ? hb.center
        : [(a.min[0] + a.max[0]) / 2, (a.min[1] + a.max[1]) / 2, (a.min[2] + a.max[2]) / 2];
      const size: [number, number, number] = hb
        ? hb.size
