  ...
  while (z > Z_BACK) {
    ...
    for (let x = -7.5; x <= 7.5 + 1e-6; x += cell) {
      ...
      if (inRing && edgeDist < 0.48) {
        ...
        col3.offsetHSL(
          (rand(seed) - 0.5) * 0.02,
          lerp(0.05, 0.16, dDepth) + (rand(seed + 9) - 0.5) * 0.04, // v43: lift NEAR saturation too (was -0.02) so the shallows don't grey out; still more saturated far
          lerp(0.06, -0.2, dDepth) + (rand(seed + 21) - 0.5) * 0.04, // darker far (depth read unchanged)
        );
