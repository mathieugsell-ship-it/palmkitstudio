  ...
  return (
    <div ref={wrapRef} className="h-full w-full bg-white">
      <Canvas
        ...
        dpr={[1, 2]}
        frameloop={frameloop}
        camera={{ position: [4.6, 3.2, 7.2], fov: 34 }}
        gl={{ antialias: true, alpha: false }}
        onCreated={({ gl, camera }) => {
          gl.setClearColor('#FFFFFF', 1);
          camera.lookAt(0.1, 1.35, 0); // frame palm + island base together
        }}
      ...
      >
        ...
        {/* Whole scene (palm + island) rotates together. */}
        <IdleRotation enabled={rotate}>
          {/* Dots live inside this rotating group → they rotate with the scene
              and are never recomputed (stable count/positions). */}
          <PalmTree config={config} bloom={bloomEnabled} pulse={rotate} />
          <IslandBase config={config} bloom={bloomEnabled} pulse={rotate} />
        </IdleRotation>
