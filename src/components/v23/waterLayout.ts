...
export function buildWater(config: PalmConfig): WaterModel {
  ...
  // BACK SHAPE: instead of a straight tongue, the back edge is a circular ARC of
  // radius R_BACK centred on the islet. It only bulges out BEHIND the islet
  // (backFactor = −sin θ, which is 0 across the whole front → no cubes added in
  // front), tapering smoothly to the ring at the sides. So the far water curves
  // like a circle behind the islet + pitons; the front stays exactly as-is.
  const R_BACK = 9.3;
  const backOuter = (theta: number) => {
    // back = 0 across the whole front (no front cubes), rising to 1 at straight
    // back. Squared so the bulge concentrates behind the islet/pitons (a clean
    // arc there) and tapers to the ring at the sides rather than ballooning.
    const back = Math.pow(Math.max(0, -Math.sin(theta)), 2);
  ...
  while (z > Z_BACK) {
    ...
    for (let x = -7.0; x <= 7.0 + 1e-6; x += cell) {
      const xc = x;
      ...
      // Beyond the ring: only the rounded BACK arc (front extension is 0).
      const inBack = !inRing && d <= backOuter(theta);
