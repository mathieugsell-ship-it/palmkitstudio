...
export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', noHover = false, labels, onHit, onBuildComplete, focusActive = false, compact = false }: Props) {
  ...
  // v64: the hero palm model as a reusable TEMPLATE (local coords, base at the
  // origin) so the focus jungle can grow smaller/slimmer copies of the SAME palm
  // (coherent with the hero), instead of inventing messy plants. foliage = frond
  // blocks (re-tinted green per variant); trunk keeps its colour.
  const palmTemplate = useMemo(() => {
    const cx = palmRig.center.x, cz = palmRig.center.z, by = palmRig.minY;
    return scene.blocks
      .filter((b) => b.group === 'palm')
      .map((b) => ({
        p: [b.position[0] - cx, b.position[1] - by, b.position[2] - cz] as [number, number, number],
        s: [b.size[0], b.size[1], b.size[2]] as [number, number, number],
        q: [b.quaternion[0], b.quaternion[1], b.quaternion[2], b.quaternion[3]] as [number, number, number, number],
        c: b.color,
        foliage: /frond/.test(b.id),
      }));
  }, [scene, palmRig]);
  const focusEaseRef = useRef(0); // eased 0→1 focus amount
  const focusSpinRef = useRef(0); // accumulating showroom rotation (rad)
  ...
  const FOCUS_SCALE = compact ? 1.06 : 1.16; // gentle showroom upscale
  const FOCUS_SPIN = 0.42; // rad/s — slow, smooth turntable
  ...
  useFrame((state, delta) => {
    ...
    // ---- v60 (INTERACTIVITY B): palm focus + scene recede ------------------
    // Ease the focus amount, then (a) glide/scale/spin the palm to its showroom
    // slot and (b) fade every non-palm block out so the palm is the clear focus.
    {
      ...
      if (builtRef.current && fa > 0.0005 && palmRig.blocks.length) {
        // accumulate the slow turntable spin (only while engaged + motion allowed)
        if (focusActive && !reducedMotion) focusSpinRef.current += delta * FOCUS_SPIN;
        else if (!focusActive && fa < 0.02) focusSpinRef.current = 0; // reset once closed
        const scaleNow = 1 + (FOCUS_SCALE - 1) * fa;
        focusScratch.fq.setFromAxisAngle(focusScratch.Y, focusSpinRef.current);
        for (const pb of palmRig.blocks) {
          const mesh = meshRefs.current[pb.i];
          if (!mesh) continue;
          // focused world pos = pivot + R_spin·(scale·(base−pivot)) + glide·fa
          focusScratch.to.copy(pb.basePos).sub(palmRig.center).multiplyScalar(scaleNow)
            .applyQuaternion(focusScratch.fq).add(palmRig.center).addScaledVector(focusGlide.current, fa);
          mesh.position.copy(pb.basePos).lerp(focusScratch.to, fa);
          focusScratch.q.copy(focusScratch.fq).multiply(pb.baseQuat);
          mesh.quaternion.copy(pb.baseQuat).slerp(focusScratch.q, fa);
        }
      }
  ...
  return (
    <group>
      ...
      {/* v62: themed FOCUS WORLD — a voxel jungle that grows around the focused
          palm (fast then slow), fades + follows it to its slot, and recedes on
          Back. Generic/reusable: each object gets its own theme later. */}
      <FocusWorld
        center={palmRig.center}
        baseY={palmRig.minY}
        radius={palmRig.radius}
        height={palmRig.height}
        scale={FOCUS_SCALE}
        glideRef={focusGlide}
        easeRef={focusEaseRef}
