...
export type SceneGroup = 'island' | 'palm' | 'water';
...
/** A service's reserved highlight node — snapped onto a real palm vertex. */
...
export function buildScene(config: PalmConfig): SceneModel {
  ...
  for (const set of [palm.blocks, island.blocks, water.blocks])
    for (const b of set) {
  ...
  const oOf = (group: SceneGroup, st: number) => {
    const base = group === 'island' ? B.islandBase : group === 'palm' ? B.palmBase : B.waterBase;
    const span = group === 'island' ? B.islandSpan : group === 'palm' ? B.palmSpan : B.waterSpan;
  ...
  // Reserved service highlight nodes (snapped onto real palm vertices by
  // buildPalm). Order follows config.hotspots so the list↔node mapping is stable.
  const anchors: SceneAnchor[] = [];
  for (const h of config.hotspots) {
    const p = palm.anchorsById[h.id];
    if (p) anchors.push({ id: h.id, position: p });
  }

  return { blocks, points, anchors };
}
