...
interface Props {
  ...
  onHit?: (group: string, service: string) => void; // v60: which object was clicked (palm → focus)
  onBuildComplete?: () => void;
  focusActive?: boolean; // v60 (INTERACTIVITY B): palm click-to-focus engaged
  compact?: boolean; // v60: phone layout → focus palm to the TOP (vs LEFT on desktop)
}
...
const ANCHOR: Record<string, [number, number, number]> = {
  ...
  boat: [-0.52, 0.09, 2.21], // v57: ON the deck amidships of the longtail hull
  //                            (the old [-0.3,0.55,2.3] sat at the boat pivot,
  //                            y=0.55 floating ~6% above the hull on screen so the
  //                            connector landed on the island/water in front)
  piton: [-6.5, 1.7, -4.3], // on the main karst column
  ship: [-1.97, 0.95, -7.34], // on the cruise-ship superstructure
};
// v56: 2D pixel offset of the PILL from the anchor dot, so the label sits OUT in
// clear negative space, joined back to its object by a longer connector line.
// v56 relocates ONLY the two that were still landing in busy content:
//   palm  — was [0,-82] (mid-fronds); now pushed up-and-right into the clear sky
//           above/beside the crown (connector drops back down to the palm).
//   boat  — was [-16,70] (on the water/island cubes); now pushed further down and
//           left into open water lower-left of the boat (connector back up to it).
// piton + ship were well placed in v55 — kept unchanged. Horizontal pushes stay
// modest so the pills stay on-screen on a portrait phone (width ~390px).
const LABEL_OFFSET: Record<string, [number, number]> = {
  palm: [64, -160],
  piton: [14, -78],
  ship: [-10, -80],
  boat: [-16, 64], // v57: re-tuned so the pill stays in the same clear lower-left
  //                  water now that the boat anchor moved down onto the hull
};

export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', noHover = false, labels, onHit, onBuildComplete, focusActive = false, compact = false }: Props) {
  ...
  // ---- v60 (INTERACTIVITY B): palm click-to-focus rig ----------------------
  // Every palm block's base (built) transform + the palm's AABB centre, used as
  // the turntable pivot. On focus the palm glides aside (left on desktop / up on
  // phone), scales up a touch and spins slowly like a showroom piece, while the
  // rest of the scene recedes. Same per-mesh approach as the boat/frond rigs.
  const palmRig = useMemo(() => {
    const blocks: { i: number; basePos: THREE.Vector3; baseQuat: THREE.Quaternion }[] = [];
    ...
    scene.blocks.forEach((b, i) => {
      ...
      for (let k = 0; k < 3; k++) {
        min[k] = Math.min(min[k], b.position[k] - b.size[k] / 2);
        max[k] = Math.max(max[k], b.position[k] + b.size[k] / 2);
      }
    });
  ...
  // v65: per-palm-block GROWTH metadata. Each palm block in LOCAL coords relative
  // to the palm base (ground centre), tagged trunk vs frond, with a trunk height
  // fraction (for the bottom→top rise) and its frond index (for staggered unfurl
  // + sway). Drives the sprout→majestic time-lapse when the palm is focused.
  const palmGrow = useMemo(() => {
    const cx = palmRig.center.x, cz = palmRig.center.z, gy = palmRig.minY;
    let trunkTop = 0.0001;
    const blocks = palmRig.blocks.map((b) => {
      const id = scene.blocks[b.i].id;
      const isTrunk = /trunk/.test(id);
      const fm = /frond-(\d+)/.exec(id);
      const local = new THREE.Vector3(b.basePos.x - cx, b.basePos.y - gy, b.basePos.z - cz);
      if (isTrunk) trunkTop = Math.max(trunkTop, local.y);
      return { i: b.i, local, baseQuat: b.baseQuat, isTrunk, frond: fm ? parseInt(fm[1], 10) : -1, yFrac: 0 };
    });
    for (const bk of blocks) bk.yFrac = bk.isTrunk ? bk.local.y / trunkTop : 1;
    return { blocks, trunkTop, fc: Math.max(config.frondCount, 1) };
  }, [palmRig, scene, config.frondCount]);
  const focusEaseRef = useRef(0); // eased 0→1 focus amount
  const growthRef = useRef(0); // v65: sprout→full growth clock (seconds, while focused)
  const focusGlide = useRef(new THREE.Vector3()); // world translation to the focus slot
  const focusScratch = useMemo(
    () => ({ Y: new THREE.Vector3(0, 1, 0), q: new THREE.Quaternion(), fq: new THREE.Quaternion(), v: new THREE.Vector3(), to: new THREE.Vector3() }),
    [],
  );
  const camera = useThree((s) => s.camera);
  const sizeW = useThree((s) => s.size.width);
  const sizeH = useThree((s) => s.size.height);
  // Compute the world glide that carries the palm's pivot to its focus slot on
  // screen (NDC target), using the live camera basis — so it stays correct across
  // the desktop/phone framings. Recomputed whenever focus engages or size flips.
  useEffect(() => {
    ...
    const d = focusScratch.v.copy(palmRig.center).sub(camera.position);
    const z = d.dot(fwd);
    const fov = (camera as THREE.PerspectiveCamera).fov ?? 34;
    const th = Math.tan((fov * Math.PI) / 180 / 2);
    const aspect = (camera as THREE.PerspectiveCamera).aspect || sizeW / sizeH;
    const ndcx = d.dot(right) / (z * th * aspect);
    const ndcy = d.dot(up) / (z * th);
    // focus slot (tunable): desktop → left & centred; phone → top.
    const tx = compact ? 0.0 : -0.46;
    const ty = compact ? 0.46 : -0.04;
    const gx = (tx - ndcx) * z * th * aspect;
    const gy = (ty - ndcy) * z * th;
    focusGlide.current.copy(right).multiplyScalar(gx).addScaledVector(up, gy);
  }, [focusActive, compact, camera, sizeW, sizeH, palmRig, focusScratch]);
  const FOCUS_SCALE = compact ? 1.06 : 1.16; // gentle focus upscale (majestic size)
  const GROW_DUR = 3.6; // seconds: sprout → full majestic (fast then slow)
  ...
  const clickables = useMemo(() => {
    ...
    return Object.entries(acc).map(([group, a]) => {
      ...
      return { group, service: CLICKABLE[group].service, key: CLICKABLE[group].key, idx: a.idx, anchor };
    });
  }, [scene]);

  // v59: shape-HUGGING hitbox geometry. A single loose box per foreground object
  // (palm crown / boat hull) projects over the background pitons + cruise ship,
  // so hovering them hits the closer foreground box. Instead, build ONE invisible
  // mesh per object whose geometry is the union of that object's own voxel blocks
  // (each grown a touch for comfy hover/tap), so the hitbox hugs the real shape
  // and never covers the empty gaps where the background peeks through. Merged
  // into one geometry per object → no flicker crossing block seams.
  const HIT_GROW = 0.16; // uniform world-space growth per voxel (≈0.08 per side)
  const hitGeoms = useMemo(() => {
    const out: Record<string, THREE.BufferGeometry> = {};
    for (const c of clickables) {
      const parts: THREE.BufferGeometry[] = [];
      for (const i of c.idx) {
        const b = scene.blocks[i];
        const g = new THREE.BoxGeometry(b.size[0] + HIT_GROW, b.size[1] + HIT_GROW, b.size[2] + HIT_GROW);
        const m = new THREE.Matrix4().compose(
          new THREE.Vector3(b.position[0], b.position[1], b.position[2]),
          new THREE.Quaternion(b.quaternion[0], b.quaternion[1], b.quaternion[2], b.quaternion[3]),
      ...
      out[c.group] = merged ?? new THREE.BufferGeometry();
    }
    return out;
  }, [clickables, scene]);
  useEffect(() => () => { Object.values(hitGeoms).forEach((g) => g.dispose()); }, [hitGeoms]);
  ...
  const setHover = (group: string, on: boolean) => {
    if (focusActive) return; // v60: no hover highlight while a service is in focus
  ...
  useFrame((state, delta) => {
    ...
    if (builtRef.current && animate && !reducedMotion && frondData.length && focusEaseRef.current < 0.001) {
      const t = state.clock.elapsedTime;
    ...
    // ---- v65: palm focus → SPROUT-TO-MAJESTIC growth + scene recede --------
    // (a) the rest of the world fades out; (b) the palm glides to its focus slot
    // while collapsing to a sprout, then GROWS: the trunk reveals bottom→top
    // (lifting the crown) and the fronds unfurl, easing fast→slow over ~GROW_DUR,
    // holding tall with a gentle frond sway. Back reverses it all (fa → 0).
    {
      const targetF = focusActive ? 1 : 0;
      const fe = focusEaseRef.current;
      const next = reducedMotion ? targetF : fe + (targetF - fe) * Math.min(1, delta * 4);
      focusEaseRef.current = Math.abs(next - targetF) < 0.0005 ? targetF : next;
      const fa = focusEaseRef.current;

      if (builtRef.current && fa > 0.0005 && palmGrow.blocks.length) {
        // growth clock: advances once the palm is mostly at its slot; resets closed.
        if (focusActive && !reducedMotion && fa > 0.5) growthRef.current += delta;
        else if (!focusActive && fa < 0.02) growthRef.current = 0;
        const gp = reducedMotion ? 1 : Math.min(growthRef.current / GROW_DUR, 1);
        const g = 1 - Math.pow(1 - gp, 3); // easeOutCubic — fast then slow
        const tp = smooth(Math.min(g / 0.6, 1)); // trunk rises over the first 60%
        const crownDrop = (1 - tp) * palmGrow.trunkTop; // crown rides the trunk top
        const cx = palmRig.center.x, cy = palmRig.center.y, cz = palmRig.center.z;
        const gy = palmRig.minY;
        const gl = focusGlide.current;
        const t = state.clock.elapsedTime;
        for (const pb of palmGrow.blocks) {
          const mesh = meshRefs.current[pb.i];
          if (!mesh) continue;
          // per-block grow factor + crown lift
          let bscale: number;
          let ly = pb.local.y;
          if (pb.isTrunk) {
            bscale = (tp * 1.05 + 0.12 - pb.yFrac) / 0.12; // bottom→top reveal (sprout keeps the base)
          } else {
            ly -= crownDrop; // frond/crown rides up with the trunk
            const fd = pb.frond >= 0 ? ((pb.frond % palmGrow.fc) / palmGrow.fc) * 0.4 : 0;
            const u = (g - 0.42 - fd) / 0.42;
            bscale = u <= 0 ? 0 : u >= 1 ? 1 : 1 + 1.70158 * Math.pow(u - 1, 3) + 0.70158 * Math.pow(u - 1, 2); // easeOutBack unfurl
          }
          bscale = bscale < 0 ? 0 : bscale > 1 ? 1 : bscale;
          // grown HOME world pos, then focus transform (spread about centre + glide)
          focusScratch.to.set(cx + pb.local.x, gy + ly, cz + pb.local.z); // grown home
          focusScratch.v.set(cx, cy, cz);
          focusScratch.to.sub(focusScratch.v).multiplyScalar(FOCUS_SCALE).add(focusScratch.v).add(gl);
          // basePos (full, home) ↔ slot, interpolated by the glide ease
          mesh.position.set(cx + pb.local.x, gy + pb.local.y, cz + pb.local.z).lerp(focusScratch.to, fa);
          mesh.scale.setScalar(Math.max(1 + (bscale - 1) * fa, 0.0001)); // collapse to sprout with the glide-in
          // gentle frond sway once grown (subtle life)
          if (!pb.isTrunk && g > 0.85 && !reducedMotion) {
            const amp = 0.03 * Math.min((g - 0.85) / 0.15, 1) * fa;
            focusScratch.fq.setFromAxisAngle(focusScratch.Y, Math.sin(t * 0.7 + pb.frond) * amp);
            mesh.quaternion.copy(focusScratch.fq).multiply(pb.baseQuat);
          } else {
            mesh.quaternion.copy(pb.baseQuat);
          }
        }
      }

      // recede the rest of the world (decor + the other 3 objects + their edges)
      if (fa > 0.0005) {
        const keep = 1 - fa;
        for (let j = 0; j < scene.blocks.length; j++) {
          if (scene.blocks[j].group === 'palm') continue;
          faceMats[j].opacity = scene.blocks[j].finalOpacity * keep;
          edgeMats[j].opacity = edgeRest[j].op * keep;
        }
      }
    }
  ...
  return (
    <group>
      ...
      {/* v65: no jungle — the focused palm itself grows (see the growth rig in
          the useFrame above). */}

      {/* v59: invisible shape-hugging hitboxes — one merged mesh per clickable
          object, geometry = union of its own voxels (so a foreground box never
          covers a background object). R3F raycasts these for hover + tap. The
          geometry already carries world positions, so the mesh sits at origin. */}
      {clickables.map((c) => (
        <mesh
          key={`hit-${c.group}`}
          geometry={hitGeoms[c.group]}
      ...
      {clickables.map((c) => {
        ...
        return (
          <Html key={`lbl-${c.group}`} position={c.anchor} occlude={false} zIndexRange={[20, 0]} style={{ pointerEvents: 'none' }}>
            <div className={`svc-pin${active ? ' is-active' : ''}${focusActive ? ' is-hidden' : ''}`} style={pinStyle}>
