...
export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', noHover = false, labels, onHit, onBuildComplete, focusActive = false, compact = false }: Props) {
  ...
  const focusSpinRef = useRef(0); // v67: accumulating showroom turntable rotation (rad)
  ...
  const FOCUS_SPIN = 0.4; // rad/s — slow, smooth showroom turntable
  ...
  useFrame((state, delta) => {
    ...
    // ---- v67: palm focus (glide + upscale + SHOWROOM SPIN + sway) + recede --
    // The rest of the world fades out; the palm glides to its slot, scales up a
    // touch and turns slowly like a showroom piece (continuous turntable about
    // its vertical axis), with the subtle frond sway layered on. Back reverses it.
    {
      ...
      if (builtRef.current && fa > 0.0005 && palmRig.blocks.length) {
        if (focusActive && !reducedMotion) focusSpinRef.current += delta * FOCUS_SPIN; // accumulate spin
        else if (!focusActive && fa < 0.02) focusSpinRef.current = 0; // reset once closed
        const cen = palmRig.center;
        const gl = focusGlide.current;
        const Rspin = focusScratch.fq.setFromAxisAngle(focusScratch.Y, focusSpinRef.current);
        const swayOn = !reducedMotion && fa > 0.6;
        const t = state.clock.elapsedTime;
        const F = config.frondMotion;
        // 1) all palm blocks → spin + upscale (about centre) + glide to slot
        for (const pb of palmRig.blocks) {
          const mesh = meshRefs.current[pb.i];
          if (!mesh) continue;
          focusScratch.to.copy(pb.basePos).sub(cen).multiplyScalar(FOCUS_SCALE).applyQuaternion(Rspin).add(cen).add(gl);
          mesh.position.copy(pb.basePos).lerp(focusScratch.to, fa);
          mesh.scale.setScalar(1);
          focusScratch.q.copy(Rspin).multiply(pb.baseQuat);
          mesh.quaternion.copy(pb.baseQuat).slerp(focusScratch.q, fa);
        }
        // 2) fronds: nod/sway in the palm frame, THEN the same spin transform
        if (swayOn && frondData.length) {
          for (const fd of frondData) {
            const sway = F.swayAmp * Math.sin(t * F.swaySpeed + fd.phase);
            const nod = F.nodAmp * Math.sin(t * F.nodSpeed + fd.phase + 0.7);
            frondScratch.q1.setFromAxisAngle(frondScratch.Y, sway);
            frondScratch.q2.setFromAxisAngle(fd.lat, nod);
            frondScratch.r.copy(frondScratch.q1).multiply(frondScratch.q2); // R_sway
            for (const bl of fd.blocks) {
              const mesh = meshRefs.current[bl.i];
              if (!mesh) continue;
              frondScratch.v.copy(bl.basePos).sub(frondOrigin).applyQuaternion(frondScratch.r).add(frondOrigin);
              focusScratch.to.copy(frondScratch.v).sub(cen).multiplyScalar(FOCUS_SCALE).applyQuaternion(Rspin).add(cen).add(gl);
              mesh.position.copy(bl.basePos).lerp(focusScratch.to, fa);
              focusScratch.q.copy(Rspin).multiply(frondScratch.r).multiply(bl.baseQuat);
              mesh.quaternion.copy(bl.baseQuat).slerp(focusScratch.q, fa);
            }
          }
        }
      }
