...
function DawnBackground({ colors, focusActive = false }: { colors: PalmConfig['colors']; focusActive?: boolean }) {
  ...
  useLayoutEffect(() => {
    ...
    // v67: in the FOCUS view we drop the opaque dawn texture so the canvas goes
    // transparent — letting the 2D foliage watermark (a DOM layer BEHIND the
    // canvas) show through behind the 3D palm. The page's .hero-dawn provides the
    // dawn gradient then. Outside focus, keep the texture (crisp, build-safe).
    scene.background = focusActive ? null : texture;
    invalidate(); // repaint in demand frameloop
  }, [scene, invalidate, size.width, size.height, texture, colors, focusActive]);
...
function CameraRig({ phone, tablet }: { phone: boolean; tablet: boolean }) {
  ...
  const invalidate = useThree((s) => s.invalidate);
  useEffect(() => {
    // phone: v54 pulls back a bit more (1.2 → 1.8) + lowers the look-at so ALL 4
    // service objects (incl. the pitons, which used to fall off the left edge) fit
    // on a portrait screen — required now that every object wears a label. Palm
    // stays central/largest. tablet: middle ground; desktop: validated framing.
    const f = phone ? 1.8 : tablet ? 1.3 : 1.0;
    camera.position.set(5.9 * f, 3.9 * f, 9.3 * f);
    camera.lookAt(0.1, phone ? 1.5 : tablet ? 1.3 : 1.05, 0);
...
// Contact shadow that smoothly fades out when a service is in focus (the island
// world recedes, so its baked shadow should melt away too) and back in on Back.
function FadingContactShadows({ focusActive, opacity = 0.28, ...props }: { focusActive: boolean; opacity?: number } & React.ComponentProps<typeof ContactShadows>) {
  const ref = useRef<THREE.Group>(null);
  const fade = useRef(1);
  useFrame((_, delta) => {
    const target = focusActive ? 0 : 1;
    fade.current += (target - fade.current) * Math.min(1, delta * 4);
    const mesh = ref.current?.children?.[0] as THREE.Mesh | undefined;
    const mat = mesh?.material as THREE.Material | undefined;
    if (mat) mat.opacity = opacity * fade.current;
  });
  return <ContactShadows ref={ref} opacity={opacity} {...props} />;
}

// ---- main island -----------------------------------------------------------
export default function PalmTreeHero({ config = DEFAULT_CONFIG, labels }: { config?: PalmConfig; labels?: Record<string, string> }) {
  ...
  // v60 (INTERACTIVITY B): click-to-focus, PALM only (service: Websites). The
  // island owns the focus flag; the palm (or its label) opens it, Esc / Back /
  // backdrop / CTA close it (the page dispatches `pk-focus-set`). We mirror the
  // state onto <html data-focus> so the page can fade the hero text + reveal the
  // service panel. Other objects don't focus yet (they keep label + hover).
  const [focus, setFocus] = useState(false);
  const handleHit = useCallback((group: string) => {
    if (group === 'palm') setFocus(true); // only the palm focuses in this step
  }, []);
  useEffect(() => {
    document.documentElement.dataset.focus = focus ? 'websites' : '';
  }, [focus]);
  useEffect(() => {
    const onSet = (e: Event) => setFocus(!!(e as CustomEvent).detail?.on);
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setFocus(false); };
    window.addEventListener('pk-focus-set', onSet as EventListener);
    window.addEventListener('keydown', onKey);
    return () => {
      window.removeEventListener('pk-focus-set', onSet as EventListener);
      window.removeEventListener('keydown', onKey);
      document.documentElement.dataset.focus = '';
    };
  }, []);

  // v53 (part A): detection feedback is now the label itself (it intensifies +
  // the object glows + console log in SceneBuild). No separate readout needed.
  ...
  return (
    <div ref={wrapRef} className="h-full w-full" style={{ background: focus ? 'transparent' : dawnBg }}>
      <Canvas
        flat
        shadows={false}
        dpr={[1, 2]}
      ...
      >
        <DemandInvalidate trigger={`${reducedMotion}-${gate}-${focus}`} />
        ...
        <DawnBackground colors={config.colors} focusActive={focus} />
