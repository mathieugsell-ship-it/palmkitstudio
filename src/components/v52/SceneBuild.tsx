...
interface Props {
  ...
  breathing?: boolean; // v52: mobile (no-hover) → permanent gentle teal breathing on the 4 objects
  onHit?: (service: string) => void; // v52: tap detection (part A: just identify, no focus yet)
...
export function SceneBuild({ config, bloom, pulse, animate, reducedMotion, gate = 'build', breathing = false, onHit, onBuildComplete }: Props) {
  ...
  const clickables = useMemo(() => {
    ...
    return Object.entries(acc).map(([group, a], oi) => {
      ...
      return { group, service: CLICKABLE[group], idx: a.idx, center, size, phase: oi * 1.7 };
    });
  }, [scene]);
  ...
  useFrame((state, delta) => {
    ...
    // ---- v52: teal "clickable" glow on the 4 objects (only after build) -----
    // Mobile (breathing): a slow, low-intensity teal breath, per-object phase so
    // they aren't in lockstep. Desktop: faint at rest, eased up on hover. A tap
    // adds a quick decaying flash. The glow tints + brightens each object's edge
    // materials toward teal (high glow overdrives luminance>1 so bloom catches it
    // on desktop). Decor edges are never touched.
    if (builtRef.current && !reducedMotion) {
      const t = state.clock.elapsedTime;
      for (const c of clickables) {
        let base;
        if (breathing) {
          base = 0.34 + 0.2 * (0.5 + 0.5 * Math.sin(t * 1.1 + c.phase)); // ~0.34..0.54, slow
        } else {
          const target = hoverRef.current[c.group] ? 0.95 : 0.14; // faint → bright on hover
          const g0 = glowRef.current[c.group] ?? 0.14;
          base = g0 + (target - g0) * Math.min(1, delta * 9); // ease
        }
        glowRef.current[c.group] = base;
        // decaying tap flash
        let fl = flashRef.current[c.group] ?? 0;
        if (fl > 0) fl = Math.max(0, fl - delta * 2.4);
        flashRef.current[c.group] = fl;
        const glow = base + fl; // can exceed 1 (flash / strong hover) → bloom on desktop
  ...
  return (
    <group>
      ...
      {/* v52: generous invisible hitboxes — one per clickable object. R3F
          raycasts these for hover (desktop) + tap (mobile). No focus yet. */}
      {clickables.map((c) => (
        <mesh
          key={`hit-${c.group}`}
          position={c.center}
          onPointerOver={(e) => {
            ...
            hoverRef.current[c.group] = true;
            if (!breathing) document.body.style.cursor = 'pointer'; // desktop pointer
          }}
          onPointerOut={(e) => {
            e.stopPropagation();
            hoverRef.current[c.group] = false;
            if (!breathing) document.body.style.cursor = '';
          }}
          onPointerDown={(e) => {
            e.stopPropagation();
            if (!builtRef.current) return; // ignore taps during the intro/build
            flashRef.current[c.group] = 1.0; // quick brighter flash
            onHit?.(c.service);
            // eslint-disable-next-line no-console
            console.log('[palmkit] hit:', c.group, '→', c.service);
          }}
        >
